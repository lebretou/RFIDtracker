...
class RFIDTag:
    def __init__(self, epc: str, tag_id: int, num_tags: int):
        ...
        self.transition_rate = 0.05  # Probability of transitioning between states
    ...
    def update_visibility(self, current_time, num_samples=10):
        ...
        # Update average of last 10 probabilities
        self.update_avg_last_10_prob()

        # Update tag state
        self.update_tag_state()

    def update_avg_last_10_prob(self):
        if len(self.prob_history) >= 10:
            self.avg_last_10_prob = np.mean(list(self.prob_history)[-10:])
        else:
